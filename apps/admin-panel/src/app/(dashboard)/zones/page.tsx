...
export default function ZonesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (err: any) { setFormError(err.message || 'Failed to save zone'); }
  };
  ...
  const handleDelete = async () => {
    ...
    catch (err: any) { setFormError(err.message || 'Failed to delete zone'); }
  ...
  const columns = [
    ...
    { key: 'actions', label: 'Actions', render: (z: any) => (
      <div className="flex gap-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(z); }} title="Delete zone" className="p-1.5 hover:bg-red-50 rounded"><Trash2 className="w-3.5 h-3.5 text-red-500" /></button>
      </div>
    )},
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {formError && !showModal && !deleteTarget && (
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" role="dialog" aria-modal="true" aria-label="Delete zone">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Delete Zone</h3>
            <p className="text-sm text-gray-600 mb-4">
              Are you sure you want to delete the zone "{deleteTarget.name}"? This cannot be undone.
            </p>
            {formError && (
              <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600" role="alert">
                {formError}
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => { setDeleteTarget(null); setFormError(''); }} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">Cancel</button>
