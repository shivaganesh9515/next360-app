...
export default function InventoryPage() {
  ...
  const columns = [
    { key: 'name', label: 'Product', render: (item: any) => (
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-slate-100 rounded-lg overflow-hidden">
          {item.images?.[0] ? <img src={item.images[0]} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-400 text-xs">Img</div>}
        </div>
    ...
    { key: 'stock', label: 'Current Stock', render: (item: any) => (
      <input type="number" defaultValue={item.stock} onBlur={(e) => { const v = parseInt(e.target.value); if (v !== item.stock) updateStock(item.id, v); }} className="w-20 px-2 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
    )},
