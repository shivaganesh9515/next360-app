...
function FooterContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('grid grid-cols-2 gap-8 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6', className)}
      {...props}
    />
  );
}
...
function FooterBottom({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'border-border text-muted-foreground mt-8 flex flex-col items-center justify-between gap-4 border-t pt-4 text-xs sm:flex-row',
        className,
      )}
      {...props}
    />
